const evaluateUnit = (purchasePrice, monthlyRent) => {
  // Acquisition Costs
...
const [priceElement, rentElement] = document.getElementsByClassName("price");

const price = Number(priceElement.innerText.match(/\d+/g).join(""));
const rent = Number(rentElement.innerText.match(/\d+/g).join(""));

evaluateUnit(price, rent);
